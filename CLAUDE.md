...
pnpm fonts       # regenerate src/styles/fonts.css — run after editing Japanese copy
```
...
whole — 11 `@font-face` blocks, ~3KB — but **Noto Sans JP is not**. Its package entry declares all
124 `unicode-range` subsets, which measured 82% of the render-blocking stylesheet's gzipped bytes for
a page that requests 28 of them, and shipped 5MB of woff2 in `dist/` regardless.

So `src/styles/globals.css` imports `./fonts.css`, which **`scripts/fonts.mjs` generates and which is
committed**. Run `pnpm fonts` after changing any Japanese copy. Measured, with the render identical to
the pixel in both themes (same screenshot sha256):

```
stylesheet   127.9KB raw / 38.9KB gzip  ->  46.5KB raw / 12.0KB gzip
woff2 in dist/  135 files / 5.12MB      ->  45 files / 0.63MB
dist/            5.8MB                  ->  1.2MB
FCP  fast 3G     980ms -> 844ms      slow 3G  3304ms -> 2776ms
CLS              0.0003 either way (the shifts are ~0.00014 each, not font swap)
```

**The subset list is derived, never written down.** Every non-ASCII character anywhere under `src/` or
in `index.html` is looked up in the package's own `unicode.json`. Scanning source text rather than
rendered output over-collects on purpose — a kanji in a comment keeps its subset — because
under-collecting costs a glyph that silently falls back to the reader's system font, and
over-collecting costs a few hundred bytes. `fonts.css` is excluded from its own corpus, or the
generated file would be an input to its own generation.

`src/styles/fonts.test.ts` is what makes a committed generated file safe, with two assertions pulling
opposite ways: **coverage** fails when a character has no block to draw it, and **no waste** fails
when a block no character needs is present. Adding a kanji outside the shipped subsets fails CI —
verified by mutation. It normalises whitespace before comparing ranges against `unicode.json`, because
`pnpm format` reformats this generated file and spaces out the comma-separated list.

`font-display: swap` is inherited from Fontsource and kept: CLS is 0.0003 on slow 3G, so a
metric-matched `size-adjust` fallback would be unverifiable complexity. Don't add one without a
measurement that justifies it.

**`tw-animate-css` was removed**, dependency and all. Nothing used an enter/exit utility — the design
has no animation — and it emitted only 24 unused `@property` declarations (211 B gzip). Tailwind
compiles an unrecognised utility to *nothing* rather than erroring, so a future shadcn component that
animates would silently render unanimated; `globals.test.ts` fails if an `animate-in`-style class
appears in `src/`, and the fix is to reinstall the package and restore its `@import`.

`shadcn/tailwind.css` stays (148 B gzip). It resolves through the package's `exports` map to
`dist/tailwind.css` — not the repo-root path the name suggests — and it is the integration point
`pnpm dlx shadcn add` expects. Its accordion and collapsible keyframes are already pruned; nothing
here uses them.
