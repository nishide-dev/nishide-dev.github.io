...
**Vite does not template `index.html`**, so its strings are duplicated in `src/data/site.ts` by
necessity. `src/site.test.ts` reads the real file and fails when the two drift — that test is the
only thing holding them together, so do not "tidy" it away.
...
`globals.css` and the words from `index.html`'s own `og:` tags so the card cannot drift from the page.
It uses the **dark** palette deliberately — a cream card all but disappears on a white Slack or X
background. Playwright is **not** a dependency: ~100MB with a browser, installed on every CI run, for
an image that changes when the name or the palette does. Run the script ad hoc from somewhere that
has it.
...
and other people's links. Its palette is inlined out of necessity: it renders before, and without,
the app. `vite preview` serves `index.html` for unknown paths, so this file can only be checked by
loading `/404.html` directly — GitHub Pages is what actually serves it.
